...
test('quota failures keep progress unchanged and can be resumed without duplicating chunks',async t=>{
 const env={JOBS_DB:database(t),GOOGLE_ANALYTICS_CONFIG:'{}'};provider(t,{transient:true});let r=await api(env,'start',{property});const failed=await handleAnalyticsReport(request('advance',{id:r.id}),env);assert.equal(failed.status,429);assert.equal((await api(env,'report?id='+r.id)).stage,0);r=await api(env,'advance',{id:r.id});assert.equal(r.stage,1);assert.equal(env.JOBS_DB.sqlite.prepare('SELECT COUNT(*) AS n FROM analytics_report_data').get().n,1);
});
